"""Per-session recordings: what Echo heard, what it said, what it did.
...
audio, compare what Echo actually did against what was wanted, and turn that
into fixes. Each session directory holds:

    mic.wav        what the mic heard (24 kHz pcm16 mono)
    echo.wav       what actually reached the speaker, chimes and zero-fill
                   included, so the timeline matches wall-clock
    session.wav    stereo review mix — left = you, right = Echo — built at
...
class SessionRecorder:
    ...
    def write_mic(self, pcm_bytes):
        """One capture block: what Echo heard."""
        if not self.closed:
            self._q.put(("mic", pcm_bytes))

    def write_echo(self, pcm_bytes):
    ...
    def close(self, end_reason=None):
        ...
        try:
            ...
            pass
        secs = max(self._frames["mic"], self._frames["echo"]) / float(self.rate)
        audio = ", %.0fs audio" % secs if secs else ""
        print("[record] saved %s (%d events%s)"
              % (self.dir, len(self.events), audio))

    def _write_mix(self):
        """session.wav: stereo review mix, left = mic (you), right = Echo.
        ...
        try:
            ...
            try:
                ...
                while done < total:
                    ...
                    for lane, track in enumerate(("mic", "echo")):
    ...
    def _write_meta(self, end_reason):
        ...
        meta = {
            ...
            "echo_s": round(self._frames["echo"] / float(self.rate), 1),
...
def render_transcript(evs, started):
    ...
    Echo actually did (tools, tasks, injections, state moves). Timestamps are
    mm:ss from session start."""
    lines = ["# Echo session — %s"
    ...
    for ev in evs:
        ...
        if t == "user_text":
            ...
            lines.append("**[%s] echo:** %s" % (at, ev.get("text", "")))
        elif t == "tool_call":
            lines.append("[%s] ⚙ %s %s"
                         % (at, ev.get("name", "?"),
