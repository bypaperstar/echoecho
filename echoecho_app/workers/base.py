...
Discovery is a package scan: every module in echoecho_app.workers registers core
kinds; every module in echoecho_app.plugins registers optional fast-path kinds
...
PLUGINS_PACKAGE = "echoecho_app.plugins"
...
def advertised():
    ...
    included only when ECHOECHO_PLUGINS=1 (they stay dispatchable regardless); a
    kind with an advertise_when predicate appears only when it returns true."""
    from echoecho_app import config  # lazy: config never imports workers
    out = []
    for _, fn in sorted(REGISTRY.items()):
        if not fn.advertise:
            continue
        if fn.is_plugin and not config.echoecho_plugins():
            continue
