#!/usr/bin/env bash
# echoctl — one entry point for Echo's lifecycle on the Mac. Used by the
# Echo.app control panel's buttons and runnable by hand:
#
#   bash scripts/echoctl.sh status|start-daemon|stop-daemon|restart-daemon
#                           boot-vm|stop-vm|reset-vm
#                           build-app|install-app|start-app|stop-app|update
#
# Daemon env pins (audio device etc.) live in ~/.echo/daemon.env — one
...
APP_DIR="$REPO/app"
VM_NAME="${ECHO_VM_NAME:-echo-vm}"
GOLDEN="${ECHO_VM_GOLDEN:-echo-golden}"
DAEMON_ENV="$HOME/.echo/daemon.env"
DAEMON_LOG="/tmp/echo-daemon.log"
APP_LOG="/tmp/echo-orb.log"
...
daemon_pid() { pgrep -f "echo\.py --voice" | head -1 || true; }
app_bundle() {
  if [ -d "/Applications/Echo.app" ]; then echo "/Applications/Echo.app";
  elif [ -d "$HOME/Applications/Echo.app" ]; then echo "$HOME/Applications/Echo.app";
  else echo ""; fi
}
...
cmd_status() {
  ...
  curl -s -o /dev/null -m 2 "http://127.0.0.1:${ECHO_VIEWER_PORT:-8765}/transcript" \
  ...
  pgrep -f "Echo\.app/Contents/MacOS/Echo" >/dev/null && echo "app: running" || echo "app: not running"
}
...
cmd_start_daemon() {
  ...
  : "${ECHO_SANDBOX:=vm}"; export ECHO_SANDBOX
  ( nohup .venv/bin/python -u echo.py --voice </dev/null >"$DAEMON_LOG" 2>&1 & )
  sleep 3
...
cmd_boot_vm() {
  ...
  ( nohup lume run "$VM_NAME" --no-display --shared-dir "$REPO/workspace:rw" </dev/null >/tmp/lume-echo-vm.log 2>&1 & )
  echo "vm booting (log /tmp/lume-echo-vm.log)"
}
...
cmd_build_app() {
  ...
  iconutil -c icns build/Echo.iconset -o build/icon.icns
  ...
  npx --yes @electron/packager . Echo --platform=darwin --arch=arm64 \
    --icon build/icon.icns --out dist --overwrite \
    --ignore '^/prototypes' --ignore '^/test' --ignore '^/dist' --ignore '^/build/Echo.iconset' \
    >/dev/null
  rm -f runtime-config.json
  echo "built $APP_DIR/dist/Echo-darwin-arm64/Echo.app"
}

cmd_install_app() {
  cmd_build_app
  target="/Applications/Echo.app"
...
case "${1:-}" in
  ...
  update)          cmd_update ;;
  *) echo "usage: echoctl.sh {status|start-daemon|stop-daemon|restart-daemon|boot-vm|stop-vm|reset-vm|build-app|install-app|start-app|stop-app|update}"; exit 2 ;;
esac
