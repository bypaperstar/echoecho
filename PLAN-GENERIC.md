# echoecho v2 — Generic Plan ("do anything," own Mac VM, open workspace)

Companion to `PLAN.md` (v0, demo-scoped). This is the design pass for the generic product: echoecho is no
...
echoecho v2 keeps the entire v0 conversation loop — Vosk "echoecho" wake, Realtime speech-to-speech
...
ladder goes from "subprocess jailed to `workspace/`" up to "echoecho's own macOS virtual machine" (Apple
...
`codex exec` — exactly the seam `workers/code_stub.py` already proves. echoecho's job is the *voice
...
- Budgets: a per-task wall-clock cap from config (`ECHOECHO_AGENT_TIMEOUT`); on breach the worker kills
...
| 2 | `vm` | **echoecho's own Mac**: macOS VM on Apple Silicon (Virtualization.framework) managed by Lume or Tart — golden image with agent CLI, browsers, dev tools preinstalled; control via SSH; virtiofs mounts | the VM. Snapshot before each task batch; rollback on "undo" | PR 12 |
...
kept resumed while echoecho runs (VMs suspend/resume in ~seconds; cold clone from golden image ~30 s).
...
- Setup names the shared folders (e.g. `~/Documents`, `~/Desktop`). They appear in the VM at
  `/Volumes/UserDocs` **read-only** (virtiofs). The echoecho workspace mounts read-write at
  `/Volumes/echoechoWorkspace` — it is the same `workspace/` the viewer renders on the host.
...
- **Open Interpreter / 01** — the open-source voice-first "language-model computer"; closest in
  spirit to echoecho's interaction model; still experimental, explicitly lacking safeguards.
...
10. **`echoecho/10-generic-core`** — registry metadata (`KIND`/`DESCRIPTION`/`ARG_SCHEMA`), generated
...
11. **`echoecho/11-long-tasks`** — progress-stream → throttled ambient injections; `needs_input`
...
12. **`echoecho/12-vm-sandbox`** — Lume lifecycle (golden image build script, clone, suspend/resume,
...
13. **`echoecho/13-user-docs`** — read-only user-folder mounts, `outbox/` convention, `outbox.apply`
    worker + backup, approval phrases in the prompt.
14. **`echoecho/14-computer-use`** (stretch) — GUI driving inside the VM (cua driver or
    screenshot+accessibility loop) for Mac-app tasks; screen-recording into the session recorder.
15. **`echoecho/15-portal-app`** — the echoecho Orb: an Electron menu-bar face for the viewer
...
3. **Voice ↔ long-task impedance** (user wakes echoecho mid-task, expects instant answers). Mitigate:
   task handles + `check_tasks` elapsed/progress; progress throttling so echoecho isn't chatty.
