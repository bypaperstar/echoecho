...
```
echoecho/
├── README.md                      # quickstart; Mac runbook (uv venv, TCC mic reset, model download, speaker-safe audio)
├── requirements.txt               # openai==2.48.0, websockets==15.0.1, vosk, recipe-scrapers (all 3.9-safe)
├── requirements-mac.txt           # + sounddevice/Vosk/LiveKit WebRTC APM on macOS
...
2. **Acoustic echo: echoecho's speaker output re-enters the mic.** Mitigated locally with WebRTC AEC fed by exactly rendered PCM, a conservative residual/tail gate, and a no-AEC playback suppression fallback; Realtime far-field noise reduction runs after that. The wake listener remains suspended during ACTIVE. Remaining risk is unusually reverberant rooms or mismatched external input/output devices, where headphones are still the safest fallback.
...
- WebRTC/Electron/browser audio transport and a TTS/STT chained fallback pipeline. (Local WebRTC APM echo cancellation now lives behind the existing PortAudio transport.)
