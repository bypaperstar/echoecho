...
```
            "echo echo" (Vosk partial) / typed line / spacebar override
  ┌──────┐ ─────────────────────────────────────────────────────────►  ┌────────┐
  │ IDLE │                                                             │ ACTIVE │
  └──────┘ ◄───────────────────────────────────┐                       └───┬────┘
...
IDLE → ACTIVE:  pause wake feed (Echo saying "echo" can't self-trigger), play chime,
...
ACTIVE:         full-duplex voice; silence clock resets on input_audio_buffer.
                ...
                (cookbook pattern).
Text mode:      identical FSM — typed "echo echo" / "that's it", stdin lines reset
...
```
 ...
 3. Client enqueues TaskRequest to orchestrator inbox and IMMEDIATELY sends
    ...
    → Echo says "On it — searching for a good pad thai" and keeps talking.
 ...
 7. Injection gate waits for a safe turn boundary: last event == response.done AND no
    speech_started without a matching speech_stopped. Then:
      ...
      + (priority==interrupt) response.create  → Echo speaks the result.
...
├── echo.py                        # entrypoint: --text/--voice/--fake-llm/--model/--script/--mic-check; wires layers, runs daemon loop
├── echo_app/
│   ├── config.py                  # env flags (ECHO_TEXT, ECHO_FAKE_LLM, ECHO_REALTIME_MODEL), paths, end-phrase regex, timeouts (silence=600s)
...
└── tests/
    ...
    ├── test_wake.py               # stream fixture WAV chunks through detector: fires on 'echo echo', silent on decoys
    ├── test_workers_fake.py       # ECHO_FAKE_LLM worker paths end-to-end to files; atomic-write behavior
...
2. **Acoustic echo: Echo's speaker output re-enters the mic** (raw WebSocket has no AEC), causing phantom barge-ins. Mitigate: hard demo-day rule — AirPods/headphones; also suspend the wake listener during ACTIVE and ignore `speech_started` <300 ms after playback begins.
...
8. **macOS mic permission (TCC) silently returning zeros.** Mitigate: `echo.py --mic-check` as step zero of setup; README documents `sudo tccutil reset Microphone com.apple.Terminal`.
